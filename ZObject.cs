    ...
    public class ZObjects
    {
        ...
        public int Id
        {
            ...
            set
            {
                ...
                {
                    fs.Seek((value + 1) * blockSize, SeekOrigin.Begin);
        ...
        private class ZObject
        {
            ...
            {
                int o = offset;
                SDL.SDL_Color b = cv.GetPalleteColor(buf[o++]);
                if (draft) b = new SDL.SDL_Color { r = 192, g = 192, b = 0, a = 255 };
                int xs = buf[o++] / 2;
                int ys = buf[o++];
                ...
                {
                    SDL.SDL_Color c = cv.GetPalleteColor(buf[o++]);
                    if (draft) c = b;
            ...
            private int OutLine(Canvas cv, int offset, SDL.SDL_Color col, int ox, int oy)
            {
                int x0, y0, x1, y1;
        ...
        private class ZTeacher : ZObject
        {
            ...
            public override void Draw(Canvas cv, bool shift = false)
            {
                ...
                {
                    x0 = r2[k];
                    SDL.SDL_Color col = new SDL.SDL_Color(){ r = 255, g = 192, b = 192, a = 255}; // { 255, 192, 192 }
                ...
                cv.drawRect(ox + 148, 78, ox + 164, 84, cv.GetPalleteColor(0));
                cv.fillRect(ox + 149, 79, ox + 163, 83, cv.GetPalleteColor(7));
                cv.fillRect(ox + 155, 78, ox + 156, 84, cv.GetPalleteColor(0));
